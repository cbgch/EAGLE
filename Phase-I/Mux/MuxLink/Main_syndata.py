...
def node_label(adj_matrix):
    subgraph = np.array(adj_matrix)
    
    K = subgraph.shape[0]
    subgraph_wo0 = subgraph[1:, 1:]
    subgraph_wo1 = subgraph[[0] + list(range(2, K)), :][:, [0] + list(range(2, K))]

    subgraph_wo0 = np.ascontiguousarray(subgraph_wo0)
    subgraph_wo1 = np.ascontiguousarray(subgraph_wo1)
...
parser.add_argument('--circuit-name',default=None,help='Circuit name')
...
if not args.only_predict:
    saveGraph=copy.deepcopy(train_graphs[0])

    with open('synData/{}/subGraph.pkl'.format(args.circuit_name), 'rb') as f:
        loaded_data = pickle.load(f)
    label_list=np.loadtxt('synData/{}/labels.txt'.format(args.circuit_name), dtype=int)

    graphNum=len(loaded_data)
    for xx in range(graphNum):
        gra=loaded_data[xx]
        nodeNum=gra.number_of_nodes()   #获取图中结点的数量
        feat_arr = np.loadtxt('synData/{}/features/features'.format(args.circuit_name)+str(xx)+'.txt', dtype=float)
        cg=copy.deepcopy(saveGraph)
        cg.num_nodes=nodeNum
        cg.node_features=feat_arr
        cg.label=int(label_list[xx])
        cg.num_edges=gra.number_of_edges()
        adj_matr=[[0 for _ in range(nodeNum)] for _ in range(nodeNum)]
        for edge in gra.edges(data=True):
            adj_matr[edge[0]][edge[1]]=1
        ...
        cg.edge_pairs=np.array(edge_vec)
        cg.degs=deg_vec
        cg.node_tags=node_label(adj_matr)

        train_graphs.append(cg)
