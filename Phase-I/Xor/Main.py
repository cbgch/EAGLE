...
logInfo = open("GNNout/log.txt","w")
answer=open("GNNout/ans.txt","w")
...
def test_new(args, model, device, test_graphs):
    ...
    # logInfo.write(str(output)+"\n")   ###################################
    for i in range(len(output)):
        logInfo.write(str(output[i][0].item())+' '+str(output[i][1].item())+'\n')

    pred = output.max(1, keepdim=True)[1]
    # logInfo.write(str(pred)+"\n")   ###################################

    labels = torch.LongTensor([graph.label for graph in test_graphs]).to(device)
    # logInfo.write(str(labels)+"\n")   ###################################
    for i in range(len(output)):
        answer.write(str(labels[i].item())+' '+str(pred[i].item())+'\n')
...
parser.add_argument('--dataset-name', default=None, help='dataset name')
...
if args.links_name is not None:
    ...
    if args.only_predict:  
        args.links_dir = os.path.join(args.file_dir, './{}/lockedNetlist/data/{}'.format(args.file_name,args.links_name))
    else:
        args.links_dir = os.path.join(args.file_dir, './{}/{}/data/{}'.format(args.file_name,args.dataset_name,args.links_name))
    links_idx = np.loadtxt(args.links_dir, dtype=int)
    links_pos = (links_idx[:, 0], links_idx[:, 1])

if args.save_model: 
    print("Reading the nodes")
    args.train_dir = os.path.join(args.file_dir, './{}/{}/data/{}'.format(args.file_name, args.dataset_name,'node_tr_pos.txt'))
    train_pos = np.loadtxt(args.train_dir, dtype=int)
    args.train_dir = os.path.join(args.file_dir, './{}/{}/data/{}'.format(args.file_name, args.dataset_name,'node_tr_neg.txt'))
    ...
    args.train_dir = os.path.join(args.file_dir, './{}/{}/data/{}'.format(args.file_name,args.dataset_name, 'node_va_pos.txt'))
    val_pos = np.loadtxt(args.train_dir, dtype=int)


    args.train_dir = os.path.join(args.file_dir, './{}/{}/data/{}'.format(args.file_name,args.dataset_name, 'node_va_neg.txt'))
    ...
    if (len(val_neg)>len(val_pos)):
        ...
        val_pos=val_temp
if args.only_predict:
    args.train_dir = os.path.join(args.file_dir, './{}/lockedNetlist/data/{}'.format(args.file_name, 'node_te_pos.txt'))
else:
    args.train_dir = os.path.join(args.file_dir, './{}/{}/data/{}'.format(args.file_name, args.dataset_name, 'node_te_pos.txt'))
...
if args.only_predict:
    args.train_dir = os.path.join(args.file_dir, './{}/lockedNetlist/data/{}'.format(args.file_name, 'node_te_neg.txt'))
else:
    args.train_dir = os.path.join(args.file_dir, './{}/{}/data/{}'.format(args.file_name, args.dataset_name, 'node_te_neg.txt'))
...
if args.only_predict:
    feats_test = np.loadtxt('./{}/lockedNetlist/data/feat.txt'.format(args.file_name), dtype='float32')
else:
    feats_test = np.loadtxt('./{}/{}/data/feat.txt'.format(args.file_name,args.dataset_name), dtype='float32')

if args.only_predict:
    count = np.loadtxt('./{}/lockedNetlist/data/count.txt'.format(args.file_name))
else:    
    count = np.loadtxt('./{}/{}/data/count.txt'.format(args.file_name,args.dataset_name))
...
if args.only_predict:
    ...
    with open('./{}/{}/data/{}_hyper.pkl'.format(args.file_name,args.dataset_name,args.data_name), 'rb') as hyperparameters_name:
    ...
    model_name = './{}/{}/data/{}_model.pth'.format(args.file_name,args.dataset_name,args.data_name)
    ...
    pred_name = './{}/{}/data/'.format(args.file_name,args.dataset_name) + 'h'+str(args.hop)+'_pred.txt'
...
for epoch in range(1, args.epochs + 1):
    ...
    if val_loss <= best_loss:
        ...
        hyper_name = './{}/{}/data/{}_hyper.pkl'.format(args.file_name,args.dataset_name,args.data_name)
